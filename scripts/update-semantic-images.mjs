...
const genericImages = {
  복지: [
    'https://images.unsplash.com/photo-1536640712-4d4c36ef0e4c?auto=format&fit=crop&q=80&w=800', // 가족
  ...
  경제: [
    'https://images.unsplash.com/photo-1580519542036-ed47f3e42218?auto=format&fit=crop&q=80&w=800', // 돈
    ...
    'https://images.unsplash.com/photo-1507679722329-873db87de647?auto=format&fit=crop&q=80&w=800', // 그래프
  ...
  생활: [
    ...
    'https://images.unsplash.com/photo-1513635269975-59663e0acad5?auto=format&fit=crop&q=80&w=800', // 카페
    'https://images.unsplash.com/photo-1506804886640-39fb4eb1a473?auto=format&fit=crop&q=80&w=800', // 도로
  ...
  행사: [
    ...
    'https://images.unsplash.com/photo-1540039155732-68746868d22c?auto=format&fit=crop&q=80&w=800', // 콘서트
    'https://images.unsplash.com/photo-1533174000222-1d54bd8c0e2c?auto=format&fit=crop&q=80&w=800', // 파티
...
async function searchWikimedia(keyword, limit = 6) {
  try {
    const url = `https://commons.wikimedia.org/w/api.php?action=query&generator=search&gsrsearch=${encodeURIComponent(keyword)}&gsrnamespace=6&gsrlimit=${limit}&prop=imageinfo&iiprop=url&format=json`;
    const res = await fetch(url);
    const data = await res.json();
    if (data && data.query && data.query.pages) {
      return Object.values(data.query.pages).map(p => p.imageinfo[0].url);
    }
  } catch (e) {
...
async function run() {
  ...
  for (const file of files) {
    ...
    if (tagMatch) {
      ...
      if (tags.length > 0) {
        firstTag = tags[0].replace(/-\s*/, '').trim();
      }
    }
